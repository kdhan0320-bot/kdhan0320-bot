...
`Figma` `HTML` `CSS` `JavaScript` `React` `Vite` `MUI` `Supabase` `GitHub Actions` `Playwright`

## 작업 방식

1. 요구사항과 사용자 흐름을 먼저 정리합니다.
2. Figma 또는 코드로 화면 구조를 구체화합니다.
3. AI 도구로 초안과 구현을 보조받습니다.
4. diff, lint, build, 실제 브라우저 화면을 직접 검토합니다.
5. 구현하지 않은 기능과 데이터 한계를 문서에 구분해 기록합니다.
